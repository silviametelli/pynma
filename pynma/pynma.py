...
from pynma._plotting_functions.cytoscape_styleesheeet import get_stylesheet
NET_DATA = pd.read_csv('db/psoriasis_wide.csv')
DEFAULT_ELEMENTS = get_network(df=NET_DATA)

from pynma.loader import Loader
import pynma._plotting_functions
...
class NMA(Loader):
    ...
    def plot_network(self, layout='circle', pie=False, height=None, width=None):
        ...
        if self._is_jupyter:
            ...
            _net_plot_path = 'pynma/__temp/export.html'
    ...
    def plot_nma_forests(self, type="nma", outcome2=False, node_ref=None, edge_ref=None):
        ...
        """
        if type == 'nma':
            fig = pynma._plotting_functions._nma_forest(node_ref=node_ref, forest_data=self.forest_data, forest_data_out2=self.funnel_data_out2)
        elif type == 'bidim':
            fig = pynma._plotting_functions._bidim_forest(node_ref=node_ref,forest_data=self.forest_data, forest_data_out2=self.funnel_data_out2)
        elif type == 'pw':
            fig = pynma._plotting_functions._pw_forest(edge_ref=edge_ref, forest_data_prws=self.forest_data_pw, forest_data_prws_out_2=self.forest_data_pw_out2)
        else:
    ...
    def league_table(self, subset=None, values_only=False, lower_error=False, upper_error=False):
        ...
        """
        return pynma._plotting_functions._print_league_table(self.net_data, self.league_table_data,
                                                                 subset=subset,
    ...
    def plot_funnels(self, node_ref, outcome2=False):
        ...
        outcome2 : bool
            Whether to show results for second outcome
        """
        fig = pynma._plotting_functions._funnelplot(node_ref=node_ref,
    ...
    def plot_ranking(self, type='heatmap', outcome2=False):
        ...
        """
        if type == 'scatter':
            fig = pynma._plotting_functions._ranking_scatter(self.net_data, self.ranking_data)
        elif type == 'heatmap':
            fig = pynma._plotting_functions._ranking_heatmap(self.ranking_data)
        else:
